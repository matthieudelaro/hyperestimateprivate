...
/**
 * @brief Tweaks the features in input by exagerating difference to mean features.
 * Implementation based on the source code of ReLULayer
 */
template <typename Dtype>
class TweakFeaturesLayer : public NeuronLayer<Dtype> {
 public:
  /**
   * @param param provides ReLUParameter relu_param,
   *     with ReLULayer options:
   ...
   *     the value @f$ \nu @f$ by which negative values are multiplied.
   */
  explicit TweakFeaturesLayer(const LayerParameter& param)
      : NeuronLayer<Dtype>(param) {}

  virtual inline const char* type() const { return "TweakFeatures"; }
 ...
 protected:
  ...
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void alterValues(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void saveValues(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  // virtual void Forward_gpu(const vector<Blob<Dtype>*>& bottom,
      // const vector<Blob<Dtype>*>& top);
