...
void convert_dataset(const string& input_folder, const string& output_folder,
  ...
  for (int itemid = 0; itemid < 5*kCIFARBatchSize; ++itemid) {
    read_image(&data_file_train, &coarse_label, &fine_label, str_buffer);
  ...
  for (int itemid = 0; itemid < kCIFARBatchSize; ++itemid) {
    read_image(&data_file_test, &coarse_label, &fine_label, str_buffer);
    datum.set_label(fine_label);
    datum.set_data(str_buffer, kCIFARImageNBytes);
    int length = snprintf(str_buffer, kCIFARImageNBytes, "%05d", itemid);
    string out;
